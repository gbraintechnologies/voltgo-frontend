...
export default function AddCardScreen() {
  ...
  const [touched, setTouched] = useState({ cardNumber: false, cardName: false, expiry: false, cvv: false });
  ...
  const errors = {
    cardNumber: touched.cardNumber && rawCard.length !== 16 ? 'Enter a valid 16-digit card number' : '',
    cardName: touched.cardName && cardName.trim().length < 3 ? 'Enter the name on your card' : '',
    expiry: touched.expiry && !validateExpiry(expiry) ? 'Enter a valid expiry date' : '',
    cvv: touched.cvv && (cvv.length < 3) ? 'CVV must be 3–4 digits' : '',
  };
  const isValid = rawCard.length === 16 && cardName.trim().length >= 3 && validateExpiry(expiry) && cvv.length >= 3;

  const borderColor = (anim: Animated.Value, hasError: boolean) =>
    hasError ? Colors.error : anim.interpolate({
      inputRange: [0, 1], outputRange: [Colors.border, Colors.borderFocus],
    });
  ...
  return (
    <View style={styles.container}>
      ...
      >
        ...
        <Animated.View style={[styles.inputWrap, { borderColor: borderColor(cardNumFocus, !!errors.cardNumber) }]}>
          <TextInput
            style={styles.input}
            ...
            onBlur={() => { animateFocus(cardNumFocus, false); setTouched(t => ({ ...t, cardNumber: true })); }}
        ...
        {/* Card Name */}
        <Text style={[styles.sectionTitle, { marginTop: 20 }]}>Name on Card</Text>
        <Animated.View style={[styles.inputWrap, { borderColor: borderColor(cardNameFocus, !!errors.cardName) }]}>
          <TextInput
            style={styles.input}
            ...
            onBlur={() => { animateFocus(cardNameFocus, false); setTouched(t => ({ ...t, cardName: true })); }}
        ...
        <View style={styles.halfRow}>
          <View style={{ flex: 1 }}>
            ...
            <Animated.View style={[styles.inputWrap, { borderColor: borderColor(expiryFocus, !!errors.expiry) }]}>
              <TextInput
                style={styles.input}
                placeholder="MM/YY"
                placeholderTextColor={Colors.textMuted}
                ...
                onBlur={() => { animateFocus(expiryFocus, false); setTouched(t => ({ ...t, expiry: true })); }}
                keyboardType="numeric"
                maxLength={5}
              />
            </Animated.View>
          ...
          <View style={{ flex: 1 }}>
            ...
            <Animated.View style={[styles.inputWrap, { borderColor: borderColor(cvvFocus, !!errors.cvv) }]}>
              <TextInput
                style={styles.input}
                placeholder="•••"
                placeholderTextColor={Colors.textMuted}
                ...
                onBlur={() => { animateFocus(cvvFocus, false); setTouched(t => ({ ...t, cvv: true })); }}
                keyboardType="numeric"
                maxLength={4}
                secureTextEntry
              />
...
const styles = StyleSheet.create({
  ...
  cardTypeBadgeText: { fontFamily: 'HelveticaNeue-CondensedBold', fontSize: 14, color: Colors.white, letterSpacing: 1 },
  cardPreviewNumber: {
    fontFamily: 'HelveticaNeue-CondensedBold', fontSize: 22,
    color: Colors.white, letterSpacing: 3, marginBottom: 24,
  },
  cardPreviewBottom: { flexDirection: 'row', justifyContent: 'space-between' },
  cardPreviewLabel: {
    fontFamily: 'Poppins-Regular', fontSize: 10,
    color: 'rgba(255,255,255,0.55)', marginBottom: 2, letterSpacing: 1,
  },
  cardPreviewValue: {
    fontFamily: 'Poppins-SemiBold', fontSize: 13, color: Colors.white,
  },
